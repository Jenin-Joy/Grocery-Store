...
def viewbooking(request): 
    ...
    cart_items = tbl_cart.objects.filter(product__seller=seller_id).select_related('booking', 'product', 'booking__user')
...
def packorder(request,id): 
    bookingdata=tbl_cart.objects.get(id=id)
    bookingdata.cart_status=3
    bookingdata.save()
...
def viewdeliveryagent(request,did):
    ...
    return render(request,'Seller/viewdeliveryagent.html',{'delivery':delivery})
